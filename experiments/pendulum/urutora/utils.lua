...
function utils.textWidth(node)
	...
	local text
	if type(node.text) == 'function' then text = node.text();
	else text = tostring(node.text); end
...
function utils.print(text, x, y)
	love.graphics.print(text, floor(x), floor(y))
end

function utils.rect(mode, a, b, c, d, cr)
	love.graphics.rectangle(mode, floor(a), floor(b), floor(c), floor(d), floor(cr))
end

function utils.line(a, b, c, d)
	love.graphics.line(floor(a), floor(b), floor(c), floor(d))
end

function utils.circ(mode, a, b, c)
	love.graphics.circle(mode, floor(a), floor(b), floor(c))
end
