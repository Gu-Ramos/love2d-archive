...
function base_node:setBounds(x, y, w, h, cr)
...
function base_node:drawBaseRectangle(color, ...)
	...
	local x, y, w, h, cr = self.x, self.y, self.w, self.h, self.cr

	if ... then x, y, w, h, cr = ... end
	utils.rect('fill', x, y, w, h, cr)
end

function base_node:drawText(color)
	local text = self.text

	if (not text) or (type(text) == 'string' and #text == 0) then
		if self.placeholder_text then
			if type(self.placeholder_text) == 'function' then
				text = self.placeholder_text()
			else
				text = tostring(self.placeholder_text)
			end
		else
			return
		end
