...
function suit:wheelmoved(dx, dy)
	self.mouseWheel_x = dx or 0
	self.mouseWheel_y = dy or 0
end
...
function suit:grabKeyboardFocus(id, x,y, w,h)
	if self:isActive(id) then
		if love._os == "Android" or love.system.getOS() == "iOS" then
			if id == NONE then
...
function suit:enterFrame()
	...
	if self.transform then
		mx, my = self.transform:inverseTransformPoint(love.mouse.getX(), love.mouse.getY())
	else
		mx, my = love.mouse.getPosition()
	end
	self:updateMouse(mx, my, love.mouse.isDown(1))
	self.key_down, self.textchar = nil, ""
	self.mouseWheel_x, self.mouseWheel_y = nil, nil
...
function suit:registerScreenDraw(f, ...)
	local args = {...}
-- 	local nargs = select('#', ...)
	self.draw_screen_queue.n = self.draw_screen_queue.n + 1
	self.draw_screen_queue[self.draw_screen_queue.n] = function()
		f(unpack6(args))
	end
end
...
function suit:draw()
	...
	for i = self.draw_screen_queue.n,1,-1 do
		self.draw_screen_queue[i]()
	end
