...
return function(core, input, ...)
	...
	opt.limit = opt.limit or math.huge
	...
	opt.hasKeyboardFocus = core:grabKeyboardFocus(opt.id, x,y,w,h)

	if (core.candidate_text.text == "") and opt.hasKeyboardFocus then
		local keycode,char = core:getPressedKey()
		-- text input
		if (char and char ~= "") and #input.text < opt.limit then
	...
	return {
		...
		submitted = core:keyPressedOn(opt.id, "return") or core:keyPressedOn(opt.id, "kpenter"),
