...
function round(x)
  return ( x - floor(x) < 0.5  and floor(x) ) or ceil(x)
end

function love.load()
  Card = love.graphics.newImage('placeholder.png')
...
function love.draw()
  for i = (n-1) - center, 0.01, -1 do
    local x = pos(1/7 * i) -- pos(1/n * i)
    local s = scale(1/n * i)
    local a = -(1/n * i)^2 + 1

    local px,py = 960 + 500 * x - 250*.3*s, 680 + (1-s)*(500*0.3)
    love.graphics.setColor(a,a,a,1)
    love.graphics.draw(Card, px, py, 0, 0.4*s)
    love.graphics.setColor(0.1*a,0.1*a,0.1*a,1)
    love.graphics.printf('Imagem', Font, px, py, 400, 'center', 0, 0.4*s, 0.4*s, -50, -77)
  end
