...
function theme.getColorForState(opt)
	return (opt.color and opt.color[opt.state]) or theme.color[opt.state or "normal"]
end

function theme.getTextColorForState(opt)
	return (opt.t_color and opt.t_color[opt.state]) or theme.color[opt.state or "normal"]
end
...
function theme.drawBox(x,y,w,h, colors, cornerRadius, drawMode, lineWidth)
	...
	local r,g,b,ta = unpack4(colors.bg or colors)
...
function theme.Label(text, opt, x,y,w,h)
	...
	local r,g,b, ta = unpack4((opt.color and opt.color.normal or NONE).fg or theme.color.normal.fg)
	local a = select(4,lg.getColor())
	a = ta and a*ta or a
	lg.setColor(r,g,b,a)
	lg.setFont(opt.font)
...
-- 	local th = opt.font:getHeight()
	...
	local text = type(chk.text) == 'function' and chk.text() or tostring(chk.text)
	local utext = type(chk.utext) == 'function' and chk.utext() or utext and tostring(chk.utext) --luacheck: ignore

	if utext and not chk.checked then
		lg.setFont(opt.font)
		y = y + theme.getVerticalOffsetForAlign(opt.valign, opt.font, h)
		lg.printf(utext, x + h, y, w - h, opt.align or "left")
	elseif text then
		lg.setFont(opt.font)
		y = y + theme.getVerticalOffsetForAlign(opt.valign, opt.font, h)
		lg.printf(text, x + h, y, w - h, opt.align or "left")
	end
end
...
function theme.Screen(border, opt, x,y,w,h)
	...
	elseif border then
		local lw = opt.lineWidth or 10
		theme.drawBox(x-lw/2,y-lw/2,w+lw,h+lw, c.fg, opt.cornerRadius+lw/2, 'line', opt.lineWidth)
	end
...
-- 	local th = opt.font:getHeight()
	...
	if chk.checked then
		theme.drawBox(x+h/10 + 20,y+h/10 + 20,h*.8-40,h*.8-40, c.fg, opt.cornerRadius, opt.drawMode, opt.lineWidth)
	end
...
function theme.Slider(fraction, opt, x,y,w,h)
	...
	if opt.fgtexture then
		...
		theme.drawBox(xb,yb,wb,hb, c.fg, opt.cornerRadius, opt.drawMode, opt.lineWidth)
	end

	if opt.state ~= nil and opt.state ~= "normal" then
		local red,g,b,ta = unpack4((opt.color and opt.color.active or NONE).fg or theme.color.active.fg)
		local a = select(4,lg.getColor())
		a = ta and a*ta or a
		lg.setColor(red,g,b,a)
		if opt.vertical then
...
function theme.Input(input, opt, x,y,w,h)
	...
	lg.intersectScissor(rx, ry, rw, rh)
	...
	if input.text == '' and input.placeholder then
		...
		lg.print(type(input.placeholder) == 'function' and input.placeholder() or tostring(input.placeholder), x, y+(h-th)/2)
	else
