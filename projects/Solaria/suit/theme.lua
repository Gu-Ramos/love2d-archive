...
local StencilShader = love.graphics.newShader[[
                                              vec4 effect(vec4 color, Image tex, vec2 tc, vec2 sc){
                                                if(texture2D(tex,tc).a == 0.0){discard;}
                                                return color*texture2D(tex,tc);
                                              }]]
...
function theme.drawBox(x,y,w,h, colors, cornerRadius, drawMode)
  ...
  love.graphics.rectangle(drawMode or 'fill', x,y, w,h, cornerRadius)
end
...
function theme.Label(text, opt, x,y,w,h)
  ...
  if opt.mesh then
    local function labeldraw()
      love.graphics.setShader(StencilShader)
      love.graphics.printf(text, opt.font, x+2, y, w-4, opt.align or "center")
      love.graphics.setShader()
    end
    love.graphics.setColor((opt.color and opt.color.normal or {}).fg or theme.color.normal.fg)
    love.graphics.stencil(labeldraw,'replace',1)
    love.graphics.setStencilTest('equal',1)
    love.graphics.draw(opt.mesh,x,ogy+4)
...
function theme.Button(text, opt, x,y,w,h)
  ...
  if opt.mesh then
    local bdraw = function()
      theme.drawBox(x,y,w,h, c, opt.cornerRadius, opt.drawMode)
    end
    love.graphics.stencil(bdraw, 'replace', 1)
    love.graphics.setStencilTest('equal',1)
    if opt.drawMode == 'line' then
      local lw = love.graphics.getLineWidth()
      love.graphics.draw(opt.mesh,x-lw,y-lw,0,1.1)
    else
      love.graphics.draw(opt.mesh,x,y)
    end
    love.graphics.setStencilTest()
  else
    theme.drawBox(x,y,w,h, c, opt.cornerRadius, opt.drawMode)
...
function theme.Checkbox(chk, opt, x,y,w,h)
  local c = theme.getColorForState(opt)
  local tc = theme.getTextColorForState(opt)
  local th = opt.font:getHeight()
  
  theme.drawBox(x+h/10,y+h/10,h*.8,h*.8, c, opt.cornerRadius, opt.drawMode)
  love.graphics.setColor(c.fg)
  if chk.checked then
...
function theme.Slider(fraction, opt, x,y,w,h)
  ...
  local c = theme.getColorForState(opt)
  theme.drawBox(x,y,w,h, c, opt.cornerRadius, opt.drawMode)
  theme.drawBox(xb,yb,wb,hb, {bg=c.fg}, opt.cornerRadius, opt.drawMode)
...
function theme.Input(input, opt, x,y,w,h)
  ...
  theme.drawBox(x,y,w,h, (opt.color and opt.color.normal) or theme.color.normal, opt.cornerRadius, opt.drawMode)
  x = x + 3
  w = w - 6
  ...
  local rx, ry = love.graphics.transformPoint(x-1, y)
  local rw, rh = love.graphics.transformPoint(x+w+2, y+h)
  rw, rh = rw-rx, rh-ry
